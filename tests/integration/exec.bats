...
@test "runc exec --preserve-fds" {
  ...
  run bash -c "echo hello > preserve-fds.test && exec 3<preserve-fds.test && $RUNC --no-sysvisorfs --log /proc/self/fd/2 --root $ROOT exec --preserve-fds=1 test_busybox cat /proc/self/fd/3"
  [ "$status" -eq 0 ]
